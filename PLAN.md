...
- `SymphonySQLiteStore`: durable local store with migrations, indexed queries, full-text search, append-only event log, sync metadata tables, and JSON payload preservation.
- `SymphonyWorkflow`: `WORKFLOW.md` discovery/loading, front matter parsing, selected-project UI diagnostics, and prompt rendering.
- `SymphonyWorkspace`: local per-task Git worktree preparation with persisted workspace cleanup policy metadata.
- `SymphonyCodexAgent`: Codex CLI runner behind the generic agent runner protocol.
- `SymphonyClaudeAgent`: Claude Code CLI runner behind the generic agent runner protocol.
- `SymphonyGeminiAgent`: Gemini CLI runner behind the generic agent runner protocol.
- `SymphonyGitHubIssuesTracker`: GitHub REST Issues adapter behind the generic tracker protocol.
...
- `SymphonySync`: provider-neutral sync outbox/cloud exchange processing, retry policy, and conflict resolution policy.
...
- [x] Prepare per-task workspace/worktree.
- [x] Track workspace path and cleanup policy.

### 4. Agent Runtime

- [x] Dispatch ready tasks into run records, workflow prompts, workspaces, and agent runners.
- [x] Add normalized agent event model coverage for tool use, partial output, completion, failures, and input requests.
- [x] Add Codex runner.
- [x] Add Claude runner.
- [x] Add Gemini runner.
...
- [x] Add cloud transport boundary.
- [x] Add Linear tracker implementation.
- [x] Add GitHub Issues tracker implementation.
